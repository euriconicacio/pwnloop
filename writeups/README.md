...
| [WingData](wingdata.md) | Hack The Box (retired) | Easy | Linux | Wing FTP 7.4.3 **CVE-2025-47812** unauth null-byte Lua RCE (`/loginok.html` → `io.popen`, `wingftp`) → readable Wing account store → `SHA256(pw+"WingFTP")` cracks `wacky` → SSH password reuse (user) → sudo root restore script `tarfile.extractall(filter="data")` under Python 3.12.3 → **CVE-2025-4517** PATH_MAX data-filter bypass writes `/etc/sudoers.d` as root → root |
| [Facts](facts.md) | Hack The Box (retired) | Easy | Linux | open CamaleonCMS `/admin/register` (captcha OCR) → low-priv user → **CVE-2026-1776** `download_private_file?file=..` arbitrary read as the app user → service unit shows app runs as **trivia** → read trivia's own `~/.ssh/id_ed25519` → crack passphrase (`ssh2john`+rockyou) → SSH (user) → `sudo facter` runs a Ruby custom fact as root (shell drops privs, in-process Ruby doesn't) → write `/etc/sudoers.d` → root |
| [AirTouch](airtouch.md) | Hack The Box (retired) | Medium | Linux / wireless | SNMP `public` leaks a consultant SSH password → container fronting 3 wireless VLANs → `hcxdumptool` clientless handshake → rockyou WPA2-PSK (`challenge`) → PSK decrypts a client's cleartext HTTP session → trusted `UserRole` cookie flipped to `admin` → `.phtml` upload RCE → commented-out password reused for a `sudo`-all Unix account → world-readable RADIUS `server.key` imported into an evil twin → harvested PEAP-MSCHAPv2 cracked (`laboratory`) → PEAP identity oracle finds `AirTouch\r4ulcl` → corp VLAN → world-readable `hostapd_wpe.eap_user` gives an `admin` password reused for a `sudo`-all account → root |
